...
### Some Theory
>At its heart, the stack is just a web-socket that connects your `app` to your `api`. When you dispatch a REDUX action then the expectation is that somewhere in your code there will be a reducer or reducers listening out for that action so that local state can be changed. Your REACT components then react to these changes and update the UI.
>
